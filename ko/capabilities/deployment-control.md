...
| Vestige | Local-first | npm으로 설치하는 단일 prebuilt Rust binary이며 local SQLite(optional SQLCipher)와 local embedding을 사용하는 local stdio MCP server로 실행됩니다. cloud, Docker, API key가 필요 없습니다. | 단일 machine의 local-only 제품이라 hosted/team deployment가 없고 embedding model을 한 번 download해야 합니다. |
